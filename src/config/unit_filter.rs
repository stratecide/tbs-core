...
use super::parse::{
    FromConfig, parse_inner_vec, parse_inner_vec_dyn, parse_tuple1, parse_tuple2, string_base,
};
    ...
    fn from_conf<'a>(
        ...
        Ok((
            match base {
                ...
                "H" | "Hero" => {
                    let (list, r) =
                        parse_inner_vec::<(HeroType, Option<u8>)>(remainder, true, loader)?;
                    remainder = r;
                ...
                "IH" | "IsHero" => {
                    let (list, r) =
                        parse_inner_vec::<(HeroType, Option<u8>)>(remainder, false, loader)?;
                    remainder = r;
                ...
                "Not" => {
                    let (list, r) = parse_inner_vec_dyn::<Self>(remainder, true, |s| {
                        Self::from_conf(s, loader)
                    })?;
                    remainder = r;
                    Self::Not(list)
                }
...
impl UnitFilter {
    pub fn check<D: Direction>(
        ...
        match self {
            ...
            Self::Tag(tags) => tags
                .iter()
                .any(|tag| unit_data.unit.get_tag(tag.0).is_some()),
            ...
            Self::Hero(h) => {
                for (_, hero, _, _, _) in heroes.get(unit_data.pos, unit_data.unit.get_owner_id()) {
                    ...
                    if h.iter()
                        .any(|h| h.0 == hero.typ() && h.1.unwrap_or(power) == power)
                    {
                        return true;
                    }
                }
            ...
            Self::IsHero(h) => {
                if let Some(hero) = unit_data.unit.get_hero() {
                    ...
                    h.len() == 0
                        || h.iter()
                            .any(|h| h.0 == hero && h.1.unwrap_or(power) == power)
                } else {
                    false
                }
            }
            Self::HeroCharge(charge_ratio) => {
                if let Some(hero) = unit_data.unit.get_hero() {
                    let environment = game.environment();
                    Rational32::new(
                        hero.get_charge() as i32,
                        hero.max_charge(&environment) as i32,
                    ) >= *charge_ratio
                } else {
                    false
                }
            ...
            Self::AttackPattern(a) => {
                let attack_type = game
                    .environment()
                    .config
                    .default_attack_pattern(unit_data.unit.typ())
                    .typ(&game.environment());
            ...
            Self::Moved => unit_data.ballast.len() > 0,
            ...
            Self::OtherUnit(filter) => match other_unit_data {
                Some(other_unit_data) => filter
                    .iter()
            ...
            Self::Not(negated) => {
                ...
                negated.iter().any(|negated| {
                    !negated.check(game, unit_data, other_unit_data, heroes, is_counter)
                })
            }
        }
    }
...
pub(crate) fn unit_filter_input<D: Direction>(
    ...
    result.insert(
        CONST_NAME_UNIT.into(),
        Dynamic::from(unit_data.unit.clone()),
    );
    result.insert(CONST_NAME_POSITION.into(), Dynamic::from(unit_data.pos));
    result.insert(
        CONST_NAME_TRANSPORT_INDEX.into(),
        dyn_opt(unit_data.unload_index.map(|i| i as i32)),
    );
    result.insert(
        CONST_NAME_TRANSPORTER.into(),
        dyn_opt(unit_data.original_transporter.map(|(u, _)| u.clone())),
    );
    result.insert(
        CONST_NAME_TRANSPORTER_POSITION.into(),
        dyn_opt(unit_data.original_transporter.map(|(_, p)| p)),
    );
    result.insert(
        CONST_NAME_OTHER_UNIT.into(),
        dyn_opt(other_unit_data.map(|ud| ud.unit.clone())),
    );
    result.insert(
        CONST_NAME_OTHER_POSITION.into(),
        dyn_opt(other_unit_data.map(|ud| ud.pos)),
    );
