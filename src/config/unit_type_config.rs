...
impl UnitTypeConfig {
    pub fn parse(data: &HashMap<UnitTypeConfigHeader, &str>) -> Result<Self, ConfigParseError> {
        ...
        let result = Self {
            ...
            movement_points: parse(data, H::MovementPoints)?,
            vision_mode: parse_def(data, H::VisionMode, VisionMode::Normal)?,
            vision: parse(data, H::Vision)?,
            true_vision: parse_def(data, H::TrueVision, 1)?,
            ...
            weapon: parse(data, H::Weapon)?,
            can_attack_after_moving: parse_def(data, H::CanAttackAfterMoving, true)?,
            attack_pattern: parse(data, H::AttackPattern)?,
