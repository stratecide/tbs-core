    ...
    fn parse(
        ...
        let result = Self {
            ...
            splash_pattern: SplashPattern {
                ...
                directions: parse_def(
                    data,
                    H::SplashDirection,
                    SplashDamageDirectionSource::AttackInput,
                    loader,
                )?,
            },
...
impl AttackConfig {
    pub(super) fn parse_deferred(&mut self, loader: &mut FileLoader) -> Result<(), Box<dyn Error>> {
        self.condition =
            parse_inner_vec(&std::mem::take(&mut self.unparsed_condition), false, loader)?.0;
        Ok(())
    }
}
...
impl TableLine for AttackSplashConfig {
    ...
    fn parse(
        ...
        let splash_type = match loader
            .splash_types
            .iter()
            .position(|s| s.as_str() == *splash_type)
        {
            Some(i) => SplashType(i),
            None => {
        ...
        let result = Self {
            ...
            direction_modifier: parse_def(
                data,
                H::DirectionModifier,
                DisplaceDirectionModifier::Keep,
                loader,
            )?,
            script,
            //custom_columns,
