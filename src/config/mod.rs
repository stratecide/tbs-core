...
use self::hero_type_config::HeroTypeConfig;
use self::commander_power_config::CommanderPowerConfig;
use self::commander_type_config::CommanderTypeConfig;
use self::commander_unit_config::CommanderPowerUnitConfig;
use self::movement_type_config::MovementPattern;
use self::terrain_type_config::TerrainTypeConfig;
use self::unit_filter::*;
use self::unit_type_config::UnitTypeConfig;
...
impl Config {
    ...
    fn unit_config(&self, typ: UnitType) -> &UnitTypeConfig {
    ...
    pub fn unit_build_overrides(&self, typ: UnitType) -> &HashSet<AttributeOverride> {
    ...
    fn hero_config(&self, typ: HeroType) -> &HeroTypeConfig {
    ...
    fn terrain_config(&self, typ: TerrainType) -> &TerrainTypeConfig {
    ...
    pub fn terrain_max_anger(&self, typ: TerrainType) -> u8 {
    ...
    fn commander_config(&self, typ: CommanderType) -> &CommanderTypeConfig {
    ...
    fn commander_unit_configs<'a, D: Direction>(&'a self, commander: &'a Commander, unit: &'a Unit<D>, game: &'a Game<D>, pos: Point) -> impl Iterator<Item = &'a CommanderPowerUnitConfig> {
    ...
    pub fn commander_unit_death_effects<D: Direction>(&self, commander: &Commander, unit: &Unit<D>, game: &Game<D>, pos: Point) -> Vec<&UnitScript> {
        ...
        for config in self.commander_unit_configs(commander, unit, game, pos) {
            result.extend(config.on_death.iter())
        }
        result
    }
    ...
    pub fn commander_unit_attack_effects<D: Direction>(&self, commander: &Commander, unit: &Unit<D>, game: &Game<D>, pos: Point) -> Vec<&AttackScript> {
        let mut result = Vec::new();
        for config in self.commander_unit_configs(commander, unit, game, pos) {
            result.extend(config.on_attack.iter())
        }
        result
    }
    ...
    pub fn commander_unit_kill_effects<D: Direction>(&self, commander: &Commander, unit: &Unit<D>, game: &Game<D>, pos: Point) -> Vec<&KillScript> {
        let mut result = Vec::new();
        for config in self.commander_unit_configs(commander, unit, game, pos) {
            result.extend(config.on_kill.iter())
        }
        result
    }
