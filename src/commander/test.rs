...
fn verify_commander_type_constants() {
    ...
    assert_eq!(
        environment.config.commander_name(CommanderType::None),
        "None"
    );
    assert_eq!(
        environment.config.commander_name(CommanderType::Zombie),
        "Zombie"
    );
    assert_eq!(
        environment.config.commander_name(CommanderType::Simo),
        "Simo"
    );
    assert_eq!(
        environment.config.commander_name(CommanderType::Vlad),
        "Vlad"
    );
    assert_eq!(
        environment.config.commander_name(CommanderType::Tapio),
        "Tapio"
    );
    assert_eq!(
        environment.config.commander_name(CommanderType::Lageos),
        "Lageos"
    );
    assert_eq!(
        environment
            .config
            .commander_name(CommanderType::SludgeMonster),
        "SludgeMonster"
    );
    assert_eq!(
        environment.config.commander_name(CommanderType::Celerity),
        "Celerity"
    );
}
...
fn zombie() {
    ...
    skull.set_tag(
        TAG_MOVEMENT_TYPE,
        TagValue::MovementType(MovementType::HOVER),
    );
    ...
    skull2.set_tag(
        TAG_MOVEMENT_TYPE,
        TagValue::MovementType(MovementType::FOOT),
    );
    ...
    for player in &map_settings.players {
        assert!(
            player
                .get_commander_options()
                .contains(&CommanderType::Zombie)
        );
    }
    map_settings.fog_mode = FogMode::Constant(FogSetting::None);
    let mut settings = map_settings.build_default();
    settings.players[0].set_commander(CommanderType::Zombie);
    let (mut server, _) = Game::new_server(map.clone(), &map_settings, settings, Urc::new(|| 0.));
    server.players.get_mut(0).unwrap().commander.charge = server
        .players
        .get_mut(0)
        .unwrap()
        .commander
    ...
    server
        .handle_command(Command::commander_power(1, Vec::new()), Urc::new(|| 0.))
        .unwrap();
    assert_eq!(server.get_tokens(Point::new(0, 4)), Vec::new());
    assert_eq!(
        server.get_unit(Point::new(0, 4)),
        Some(
            &UnitType::MARINE
                .instance(&environment)
                .set_owner_id(0)
                .set_hp(50)
                .set_flag(FLAG_ZOMBIFIED)
                .set_movement_type(MovementType::HOVER)
                .build()
        )
    );
    assert_eq!(
        server.get_unit(Point::new(1, 4)),
        Some(
            &UnitType::MARINE
                .instance(&environment)
                .set_owner_id(0)
                .set_hp(50)
                .set_flag(FLAG_ZOMBIFIED)
                .set_movement_type(MovementType::FOOT)
                .build()
        )
    );
    server
    ...
    let mut server = unchanged.clone();
    server
        .handle_command(Command::commander_power(2, Vec::new()), Urc::new(|| 0.))
        .unwrap();
    assert_eq!(server.get_tokens(Point::new(0, 4)), Vec::new());
    assert_eq!(
        server.get_unit(Point::new(0, 4)),
    ...
    server
        ...
        .get_mut(0)
        .unwrap()
        .set_tag(&environment, TAG_FUNDS, 1000.into());
    server
        .handle_command(
...
fn simo() {
    ...
    assert!(
        arty.shortest_path_to_attack(&board, &Path::new(arty_pos), None, target_close)
            .is_some()
    );
    assert_eq!(
        arty.shortest_path_to_attack(&board, &Path::new(arty_pos), None, target_far),
        None
    );
    server
        .handle_command(
    ...
    server
        ...
        .unwrap();
    server
        .handle_command(Command::commander_power(4, Vec::new()), Urc::new(|| 0.))
        .err()
        .unwrap();
    server
        .handle_command(Command::commander_power(5, Vec::new()), Urc::new(|| 0.))
        .err()
        .unwrap();
    let arty = server.get_unit(arty_pos).unwrap();
    let board = Board::new(&server);
    ...
    server
        ...
        .unwrap();
    server
        .handle_command(Command::commander_power(3, Vec::new()), Urc::new(|| 0.))
        .unwrap();
    //let arty = server.get_unit(arty_pos).unwrap();
    server
        .handle_command(
        ...
        .err()
        .expect("range shouldn't be increased");
    server
        .handle_command(
            Command::UnitCommand(UnitCommand {
                unload_index: None,
    ...
    assert!(
        arty.shortest_path_to_attack(&board, &Path::new(arty_pos), None, target_farthest)
            .is_none()
    );
    server
        .handle_command(
...
fn tapio() {
    ...
    map.set_terrain(
        Point::new(0, 1),
        TerrainType::Grass.instance(&map_env).build(),
    );
    map.set_terrain(
        Point::new(0, 2),
        TerrainType::Forest.instance(&map_env).build(),
    );
    map.set_terrain(
        Point::new(0, 3),
        TerrainType::FairyForest
            .instance(&map_env)
            .set_owner_id(0)
            .build(),
    );
    ...
    assert!(
        server.get_unit(Point::new(1, 0)).unwrap().get_hp()
            > server.get_unit(Point::new(1, 1)).unwrap().get_hp(),
        "stronger attack from grass than street"
    );
    assert!(
        server.get_unit(Point::new(1, 1)).unwrap().get_hp()
            > server.get_unit(Point::new(1, 2)).unwrap().get_hp(),
        "stronger attack from forest than grass"
    );
    assert_eq!(
        server.get_unit(Point::new(1, 2)).unwrap().get_hp(),
        server.get_unit(Point::new(1, 3)).unwrap().get_hp(),
        "fairy forest == normal forest"
    );
    let fairy_forest_hp = server.get_unit(Point::new(0, 3)).unwrap().get_hp();

    // fairy forest heals
    server
        .handle_command(Command::EndTurn, Urc::new(|| 0.))
        .unwrap();
    assert!(
        server.get_unit(Point::new(5, 4)).unwrap().get_hp() > 1,
        "heals even enemy units"
    );
    assert_eq!(
        server.get_unit(Point::new(0, 3)).unwrap().get_hp(),
        fairy_forest_hp,
        "only heal on your own start turn event"
    );
    server
        .handle_command(Command::EndTurn, Urc::new(|| 0.))
        .unwrap();
    assert!(
        server.get_unit(Point::new(0, 3)).unwrap().get_hp() > fairy_forest_hp,
        "heal on your own start turn event"
    );
    ...
    server
        .handle_command(
            Command::commander_power(1, vec![CustomActionInput::Point(Point::new(3, 0))]),
            Urc::new(|| 0.),
        )
        .err()
        .expect("can't turn street into fairy forest");
    for i in 0..2 {
        let charge_before = server.players[0].commander.charge;
        server
            .handle_command(
                Command::commander_power(1, vec![CustomActionInput::Point(Point::new(2, i))]),
                Urc::new(|| 0.),
            )
            .expect(&format!("loop {i}"));
        assert!(server.players[0].commander.charge < charge_before);
        assert_eq!(
            server.get_terrain(Point::new(2, i)).unwrap().typ(),
            TerrainType::FairyForest
        );
    }
    ...
    server
        ...
        .unwrap();
    assert_eq!(
        server.get_terrain(Point::new(5, 3)).unwrap().typ(),
        TerrainType::Grass
    );
    assert_eq!(
        server.get_terrain(Point::new(3, 0)).unwrap().typ(),
        TerrainType::FairyForest,
        "power doesn't affect others players' fairy forests"
    );
    assert_eq!(
        server.get_unit(Point::new(0, 2)).unwrap().get_hp(),
        100,
        "don't hurt your own units"
    );
    ...
    server
        ...
        .get_mut(0)
        .unwrap()
        .set_tag(&environment, TAG_FUNDS, 10000.into());
    server
        .handle_command(Command::EndTurn, Urc::new(|| 0.))
        .unwrap();
    server
        ...
        .unwrap();
    assert_ne!(
        server.get_fog_at(ClientPerspective::Team(0), Point::new(5, 3)),
...
fn sludge_monster() {
    ...
    for player in &map_settings.players {
        assert!(
            player
                .get_commander_options()
                .contains(&CommanderType::SludgeMonster)
        );
    }
    map_settings.fog_mode = FogMode::Constant(FogSetting::None);
    let mut settings = map_settings.build_default();
    ...
    server
        ...
        .unwrap();
    assert_eq!(
        server.get_unit(p_attack_owned).unwrap().get_hp(),
        100 - base_dmg_owned
    );
    ...
    for p in get_neighbors(
        &server,
        p_sludge_owned,
        crate::map::map::NeighborMode::Direct,
    ) {
    ...
    assert!(
        !server
            .get_unit(p_die_enemy)
            .unwrap()
            .options_after_path(&board, &path, None, &[])
            .contains(&UnitAction::custom(CA_UNIT_CLEAN_SLUDGE, Vec::new()))
    );
    let path = Path::new(p_sludge_enemy);
    let options =
        server
            .get_unit(p_sludge_enemy)
            .unwrap()
            .options_after_path(&board, &path, None, &[]);
    assert!(
        options.contains(&UnitAction::custom(CA_UNIT_CLEAN_SLUDGE, Vec::new())),
        "{options:?}"
    );
    server
        .handle_command(
...
fn celerity() {
    ...
    for player in &map_settings.players {
        assert!(
            player
                .get_commander_options()
                .contains(&CommanderType::Celerity)
        );
    }
    map_settings.fog_mode = FogMode::Constant(FogSetting::None);
    let funds = 10000;
    map_settings.players[0]
        .get_tag_bag_mut()
        .set_tag(&map_env, TAG_FUNDS, funds.into());

    // get some default values without using Celerity
    let (mut server, _) = Game::new_server(
        map.clone(),
        &map_settings,
        map_settings.build_default(),
        Urc::new(|| 0.),
    );
    server
        .handle_command(
    ...
    for i in 0..=3 {
            ...
            server
                .handle_command(
                    Command::UnitCommand(UnitCommand {
                        unload_index: None,
                        path: Path::new(Point::new(2, 2)),
                        action: UnitAction::Attack(AttackInput::SplashPattern(
                            OrientedPoint::simple(target, d),
                        )),
                    }),
                    Urc::new(|| 0.),
                )
        ...
        if i > 0 {
            ...
            assert_eq!(
                server
                    .get_unit(Point::new(2, 2))
                    .unwrap()
                    .get_tag(TAG_LEVEL),
                Some(TagValue::Int(Int32(i as i32)))
            );
        }
    ...
    for i in 0..attack_damage.len() - 1 {
        assert!(
            attack_damage[i] < attack_damage[i + 1],
            "attack damage by level: {:?}",
            attack_damage
        );
    }
    ...
    assert_eq!(
        default_attack,
        100 - server.get_unit(Point::new(3, 2)).unwrap().get_hp()
    );
    ...
    let convoy: Unit<Direction6> = UnitType::CONVOY
        .instance(&environment)
        .set_owner_id(0)
        .set_transported(vec![
    ...
    let imported =
        Game::import_server(exported, &config, Version::parse(VERSION).unwrap()).unwrap();
    assert_eq!(imported, unchanged);
    assert_eq!(
        server.get_unit(Point::new(4, 0)),
        Some(
            &UnitType::CONVOY
                .instance(&environment)
                .set_owner_id(0)
...
fn lageos() {
    ...
    let neutral_vision_range = (0..map.width())
        .find(|x| {
            server.get_fog_at(ClientPerspective::Team(0), Point::new(*x, 0)) == FogIntensity::Dark
        })
        .unwrap();
    ...
    assert!(
        damage_to_neutral_heli > damage_to_lageos_heli,
        "{damage_to_neutral_heli} > {damage_to_lageos_heli}"
    );
    assert_eq!(
        damage_to_neutral_tank,
        100 - server.get_unit(Point::new(1, 1)).unwrap().get_hp()
    );
    assert!(
        neutral_vision_range
            < (0..map.width())
                .find(
                    |x| server.get_fog_at(ClientPerspective::Team(0), Point::new(*x, 0))
                        == FogIntensity::Dark
                )
                .unwrap()
    );
    ...
    assert_eq!(
        3 * (100 - server.get_unit(Point::new(0, 1)).unwrap().get_hp()),
        2 * (100 - server.get_unit(Point::new(1, 1)).unwrap().get_hp())
    );
    assert!(
        !server
            .get_unit(Point::new(0, 1))
            .unwrap()
            .has_flag(FLAG_EXHAUSTED)
    );
    ...
    server
        .handle_command(
            Command::commander_power(2, vec![CustomActionInput::Point(Point::new(0, 1))]),
            Urc::new(|| 0.),
        )
        .unwrap();
    assert_eq!(server.get_unit(Point::new(1, 0)).unwrap().get_hp(), 100);
