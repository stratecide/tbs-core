...
fn simo() {
    ...
    let mut server = unchanged.clone();
    server.handle_command(Command::CommanderPower(1, Vec::new()), || 0.).unwrap();
    server.handle_command(Command::CommanderPower(4, Vec::new()), || 0.).err().unwrap();
    server.handle_command(Command::CommanderPower(5, Vec::new()), || 0.).err().unwrap();
    let arty = server.get_unit(arty_pos).unwrap();
    assert!(arty.shortest_path_to_attack(&server, &Path::new(arty_pos), None, target_close).is_some());
    assert!(arty.shortest_path_to_attack(&server, &Path::new(arty_pos), None, target_far).is_none());
    server.handle_command(Command::UnitCommand(UnitCommand {
    ...
    let mut server = unchanged.clone();
    server.handle_command(Command::CommanderPower(1, Vec::new()), || 0.).unwrap();
    server.handle_command(Command::CommanderPower(3, Vec::new()), || 0.).unwrap();
    //let arty = server.get_unit(arty_pos).unwrap();
    server.handle_command(Command::UnitCommand(UnitCommand {
        unload_index: None,
        path: Path::new(arty_pos),
    ...
    let mut server = unchanged.clone();
    server.handle_command(Command::CommanderPower(2, Vec::new()), || 0.).unwrap();
    server.handle_command(Command::CommanderPower(3, Vec::new()), || 0.).err().unwrap();
    server.handle_command(Command::CommanderPower(4, Vec::new()), || 0.).unwrap();
    let arty = server.get_unit(arty_pos).unwrap();
    ...
    let mut server = unchanged.clone();
    server.handle_command(Command::CommanderPower(2, Vec::new()), || 0.).unwrap();
    server.handle_command(Command::CommanderPower(5, Vec::new()), || 0.).unwrap();
    let arty = server.get_unit(arty_pos).unwrap();
    assert!(arty.shortest_path_to_attack(&server, &Path::new(arty_pos), None, target_close).is_some());
    assert!(arty.shortest_path_to_attack(&server, &Path::new(arty_pos), None, target_far).is_some());
...
fn tapio() {
    ...
    let mut server = unchanged.clone();
    server.handle_command(Command::CommanderPower(1, vec![CustomActionData::Point(Point::new(3, 0))]), || 0.).err().expect("can't turn street into fairy forest");
    for i in 0..2 {
        let charge_before = server.players[0].commander.charge;
        server.handle_command(Command::CommanderPower(1, vec![CustomActionData::Point(Point::new(2, i))]), || 0.).expect(&format!("loop {i}"));
    ...
    server.handle_command(Command::BuyUnit(Point::new(5, 3), UnitType::SmallTank, Direction4::D0), || 0.).err().unwrap();
    server.handle_command(Command::CommanderPower(3, Vec::new()), || 0.).unwrap();
    assert_eq!(server.get_fog_at(ClientPerspective::Team(0), Point::new(5, 3)), FogIntensity::TrueSight);
    server.handle_command(Command::BuyUnit(Point::new(5, 3), UnitType::SmallTank, Direction4::D0), || 0.).unwrap();
