...
impl FogSetting {
    pub const GRADIENT_WITH_NONE: &'static [Self] =
        &[Self::None, Self::Sharp(2), Self::Sharp(1), Self::Sharp(0)];
    pub const GRADIENT_DARK: &'static [Self] =
        &[Self::Fade1(2), Self::Fade2(1), Self::ExtraDark(0)];
    pub const GRADIENT_LIGHT: &'static [Self] = &[Self::Light(0), Self::Fade2(3), Self::Fade2(1)];
...
impl FogMode {
    ...
    pub fn fog_setting(&self, turn: usize, player_count: usize) -> FogSetting {
        let (bright_duration, dark_duration, start_dark, gradient) = match self {
            ...
            Self::GradientWithNone(bright_duration, dark_duration, start_dark) => (
                bright_duration,
                dark_duration,
                start_dark,
                FogSetting::GRADIENT_WITH_NONE,
            ),
            Self::GradientDark(bright_duration, dark_duration, start_dark) => (
                bright_duration,
                dark_duration,
                start_dark,
                FogSetting::GRADIENT_DARK,
            ),
            Self::GradientLight(bright_duration, dark_duration, start_dark) => (
                bright_duration,
                dark_duration,
                start_dark,
                FogSetting::GRADIENT_LIGHT,
            ),
            Self::GradientLarge(bright_duration, dark_duration, start_dark) => (
                bright_duration,
                dark_duration,
                start_dark,
                FogSetting::GRADIENT_LARGE,
            ),
        };
        gradient_progress(
            gradient,
    ...
    pub fn turns_until_repeat(&self, player_count: usize) -> usize {
        let (bright_duration, dark_duration, gradient) = match self {
            ...
            Self::GradientWithNone(bright_duration, dark_duration, _) => (
                bright_duration,
                dark_duration,
                FogSetting::GRADIENT_WITH_NONE,
            ),
            Self::GradientDark(bright_duration, dark_duration, _) => {
                (bright_duration, dark_duration, FogSetting::GRADIENT_DARK)
            }
            Self::GradientLight(bright_duration, dark_duration, _) => {
                (bright_duration, dark_duration, FogSetting::GRADIENT_LIGHT)
            }
            Self::GradientLarge(bright_duration, dark_duration, _) => {
                (bright_duration, dark_duration, FogSetting::GRADIENT_LARGE)
            }
        };
        **bright_duration as usize
            + **dark_duration as usize
            + 2 * intermediate_turns(gradient, player_count)
    }
}
...
fn intermediate_turns(gradient: &[FogSetting], player_count: usize) -> usize {
    if gradient.len() <= 2 {
        panic!(
            "not much of a fog gradient when there are only {} steps",
            gradient.len()
        );
    }
...
fn gradient_progress(
    gradient: &[FogSetting],
    bright_duration: FogDuration,
    dark_duration: FogDuration,
    start_dark: bool,
    turn: usize,
    player_count: usize,
) -> FogSetting {
    ...
    let cycle_duration =
        *bright_duration as usize + *dark_duration as usize + 2 * gradient_duration;
...
pub fn get_visible_unit<D: Direction>(
    board: &Board<D>,
    team: ClientPerspective,
    p: Point,
) -> Option<Unit<D>> {
    board.get_unit(p).and_then(|u| {
...
pub fn recalculate_fog<D: Direction>(
    game: &Board<D>,
    perspective: ClientPerspective,
) -> FxHashMap<Point, FogIntensity> {
...
pub fn add_vision(
    vision: &mut FxHashMap<Point, FogIntensity>,
    to_add: &FxHashMap<Point, FogIntensity>,
) {
...
pub fn can_see_unit_at<D: Direction>(
    game: &Board<D>,
    team: ClientPerspective,
    position: Point,
    unit: &Unit<D>,
...
pub fn is_unit_visible<D: Direction>(
    board: &Board<D>,
    unit: &Unit<D>,
    p: Point,
    team: ClientPerspective,
...
pub fn is_unit_attribute_visible(
    fog_intensity: FogIntensity,
    unit_visibility: UnitVisibility,
    attribute_visibility: UnitVisibility,
) -> bool {
    match fog_intensity {
        FogIntensity::TrueSight => true,
        FogIntensity::NormalVision => {
            unit_visibility != UnitVisibility::Stealth
                && attribute_visibility != UnitVisibility::Stealth
        }
        FogIntensity::Light => {
        ...
        FogIntensity::Dark => {
            unit_visibility == UnitVisibility::AlwaysVisible
                && attribute_visibility != UnitVisibility::Stealth
        }
    }
}

pub fn visible_unit_with_attribute<D: Direction>(
    game: &Board<D>,
    team: ClientPerspective,
    pos: Point,
    attribute_visibility: UnitVisibility,
...
pub fn visible_unit_with_attribute_transported<D: Direction>(
    game: &Board<D>,
    team: ClientPerspective,
    pos: Point,
    unload_index: usize,
    ...
    let transport_visibility =
        transporter
            .environment()
            .unit_transport_visibility(game, &transporter, pos, &[]);
    ...
    transporter
        ...
        .filter_map(|(i, unit)| {
            ...
            match fog_intensity {
                ...
                FogIntensity::NormalVision | FogIntensity::Light => {
