...
impl<'a, D: Direction> EventHandler<'a, D> {
    ...
    pub fn observe_unit(&mut self, position: Point, unload_index: Option<usize>) -> UnitId<D> {
        if let Some((id, (_, _, distortion))) = self
            .observed_units
            .iter()
            .find(|(_, (p, i, _))| *p == position && *i == unload_index)
        {
            ...
            self.observed_units.insert(
                self.next_observed_unit_id,
                (position, unload_index, Distortion::neutral()),
            );
    ...
    pub fn move_observed_unit(
        ...
        self.observed_units
            .insert(id, (p, unload_index, distortion));
    }

    fn add_client_events(
        &mut self,
        f: impl Fn(ClientPerspective, &Game<D>, &Board<D>, &mut Vec<Event<D>>),
    ) {
    ...
    pub fn end_turn(&mut self) {
        ...
        let fog_before = if self.get_game().has_secrets() {
            let team = self
                .get_game()
                .players
                .get((self.get_game().current_turn() + 1) % self.get_game().players.len())
                .unwrap()
                .get_team();
        ...
        for p in valid_points(self.get_game()) {
            if let Some(hero) = self
                ...
                .get_map()
                .get_unit(p)
                .filter(|u| u.get_owner_id() == owner_id)
                .and_then(|u| u.get_hero())
                .cloned()
            {
                ...
                if hero.can_activate_power(self.environment(), next_power, true) {
                    ...
                    self.add_hero_charge(
                        p,
                        None,
                        -(hero.power_cost(self.environment(), next_power) as i32),
                    );
    ...
    pub fn start_turn(&mut self, fog_before: Option<HashMap<Point, FogIntensity>>) {
        ...
        if was_foggy != self.get_game().has_secrets() {
            ...
            self.add_client_events(|perspective, game, _, events| {
                for player in game.players.iter().filter(|p| p.get_team() != perspective) {
                    ...
                    {
                        events.push(Event::PlayerFlag(
                            Owner(player.get_owner_id()),
                            FlagKey(*flag),
                        ));
                    }
                    ...
                    {
                        if was_foggy {
                            events.push(Event::PlayerSetTag(
                                Owner(player.get_owner_id()),
                                TagKeyValues(TagKey(*key), [value.clone()]),
                            ));
                        } else {
                            events.push(Event::PlayerRemoveTag(
                                Owner(player.get_owner_id()),
                                TagKeyValues(TagKey(*key), [value.clone()]),
                            ));
                        }
    ...
    pub fn recalculate_fog(&mut self) {
        ...
        let mut perspectives: HashSet<ClientPerspective> = self
            .get_game()
            .get_teams()
            .into_iter()
    ...
    pub fn change_fog(&mut self, team: ClientPerspective, changes: HashMap<Point, FogIntensity>) {
        let changes: Vec<(Point, FogIntensity, FogIntensity)> = changes
            .into_iter()
            .map(|(p, intensity)| (p, self.get_game().get_fog_at(team, p), intensity))
            .filter(|(_, before, after)| before != after)
            .collect();
        if changes.len() > 0 {
            self.add_event(Event::PureFogChange(
                from_client_perspective(team).into(),
    ...
    pub fn add_commander_charge(&mut self, owner: i8, delta: i32) {
        if let Some(player) = self.get_game().get_owning_player(owner).cloned() {
            ...
            let delta = delta
                .max(-(player.commander.get_charge() as i32))
                .min((player.commander.get_max_charge() - player.commander.get_charge()) as i32);
            if delta != 0 {
    ...
    pub fn commander_power(&mut self, owner: i8, index: usize) {
        if let Some(player) = self.get_game().get_owning_player(owner).cloned() {
            if player.commander.get_active_power() != index {
                self.add_event(Event::CommanderPowerIndex(
                    owner.into(),
                    player.commander.get_active_power().into(),
                    index.into(),
                ));
    ...
    pub fn token_add(&mut self, position: Point, token: Token<D>) {
        ...
        let mut tokens: Vec<Token<D>> = old_tokens
            .iter()
        ...
        if tokens.len() < MAX_STACK_SIZE as usize {
            tokens.push(token);
            self.add_event(Event::ReplaceToken(
                position,
                old_tokens.try_into().unwrap(),
                Token::correct_stack(tokens).try_into().unwrap(),
            ));
    ...
    pub fn token_remove(&mut self, position: Point, index: usize) {
        match self.get_game().get_tokens(position).get(index).cloned() {
            Some(token) => {
    ...
    pub fn unit_set_hero(&mut self, position: Point, hero: Hero) {
        let unit = self
            .get_game()
            .get_unit(position)
            .expect(&format!("Missing unit at {:?}", position))
            .clone();
        if !unit.is_hero() {
    ...
    pub fn add_hero_charge(&mut self, position: Point, unload_index: Option<usize>, delta: i32) {
        ...
        let delta = delta
            .max(-(hero.get_charge() as i32))
            .min((hero.typ().max_charge(self.environment()) - hero.get_charge()) as i32);
        if delta != 0 {
            if let Some(unload_index) = unload_index {
                self.add_event(Event::HeroChargeTransported(
                    position,
                    unload_index.into(),
                    delta.into(),
                ));
    ...
    pub fn hero_power(&mut self, position: Point, index: usize) {
        ...
        if hero.get_active_power() != index {
            self.add_event(Event::HeroPower(
                position,
                hero.get_active_power().into(),
                index.into(),
            ));
            self.update_cached_visibility(position);
        }
    ...
    pub fn player_dies(&mut self, owner_id: i8) {
        if self
            .get_game()
            .get_owning_player(owner_id)
            .map(|player| !player.dead)
            .unwrap_or(false)
        {
    ...
    pub fn set_player_tag(&mut self, owner_id: i8, key: usize, value: TagValue<D>) {
            ...
            .map(|p| p.get_tag(key))
        {
            ...
            Some(None) => self.add_event(Event::PlayerSetTag(
                Owner(owner_id),
                TagKeyValues(TagKey(key), [value]),
            )),
            Some(Some(old)) => {
                if old != value {
                    self.add_event(Event::PlayerReplaceTag(
                        Owner(owner_id),
                        TagKeyValues(TagKey(key), [old, value]),
                    ));
                }
            }
        }
    ...
    pub fn remove_player_tag(&mut self, owner_id: i8, key: usize) {
        if let Some(old) = self
            .get_game()
            .get_owning_player(owner_id)
            .and_then(|p| p.get_tag(key))
        {
            self.add_event(Event::PlayerRemoveTag(
                Owner(owner_id),
                TagKeyValues(TagKey(key), [old]),
            ));
        }
    }

    pub fn terrain_replace(&mut self, position: Point, terrain: Terrain<D>) {
        let old_terrain = self
            .get_game()
            .get_terrain(position)
            .expect(&format!("Missing terrain at {:?}", position))
            .clone();
    ...
    pub fn set_terrain_tag(&mut self, position: Point, key: usize, value: TagValue<D>) {
        ...
        match terrain.get_tag(key) {
            Some(old) => {
                self.add_event(Event::TerrainReplaceTag(
                    position,
                    TagKeyValues(TagKey(key), [old, value]),
                ));
            }
            _ => {
                self.add_event(Event::TerrainSetTag(
                    position,
                    TagKeyValues(TagKey(key), [value]),
                ));
            }
    ...
    pub fn remove_terrain_tag(&mut self, position: Point, key: usize) {
        ...
        if let Some(old) = terrain.get_tag(key) {
            self.add_event(Event::TerrainRemoveTag(
                position,
                TagKeyValues(TagKey(key), [old]),
            ));
        }
    ...
    pub fn unit_creation(&mut self, position: Point, unit: Unit<D>) {
        ...
        if let ClientPerspective::Team(team) = unit.get_team() {
            if self.get_game().has_secrets() && self.get_game().is_team_alive(team) {
                ...
                let changes = unit
                    .get_vision(self.get_board(), position, &heroes)
                    .into_iter()
                    .filter(|(p, intensity)| {
                        *intensity
                            < self
                                .get_game()
                                .get_fog_at(ClientPerspective::Team(team), *p)
                    })
                    .collect();
    ...
    pub fn unit_add_transported(&mut self, position: Point, mut unit: Unit<D>) {
        if let Some(transporter) = self.game.get_unit(position) {
            unit.cached_visibility = Some(self.board.environment().config.unit_visibility(
                &self.board,
                &unit,
                position,
                Some((transporter, transporter.get_transported().len())),
            ));
    ...
    pub fn unit_path(
        ...
        let mut unit = self
            .get_game()
            .get_unit(path.start)
            .expect(&format!("Missing unit at {:?}", path.start))
            .clone();
        if let Some(unload_index) = unload_index {
            if let Some(u) = unit.get_transported().get(unload_index) {
                self.add_event(Event::UnitRemoveBoarded(
                    path.start,
                    unload_index.into(),
                    u.clone(),
                ));
        ...
        let (effect, transformed_unit, vision_changes) =
            self.animate_unit_path(&unit, path, involuntarily);
        self.effect(effect);
        let (path_end, distortion) = path.end(&*self.get_game()).unwrap();
        if board_at_the_end {
            self.move_observed_unit(
                unit_id,
                path_end,
                Some(
                    self.get_game()
        ...
        for p in path.points(self.get_game()).unwrap() {
            for token in self.get_game().get_tokens(p) {
                if let Some(function_index) =
                    self.environment().config.token_on_unit_path(token.typ())
                {
    ...
    pub fn animate_unit_path(
        &self,
        unit: &Unit<D>,
        path: &Path<D>,
        involuntarily: bool,
    ) -> (Effect<D>, Unit<D>, HashMap<Point, FogIntensity>) {
        let unit_team = unit.get_team();
        ...
        for (i, step) in path.steps.iter().enumerate() {
            if self.get_game().has_secrets()
                && !involuntarily
                && (i == 0 || transformed_unit.vision_mode().see_while_moving())
            {
                ...
                for (p, vision) in transformed_unit.get_vision(self.get_board(), current, &heroes) {
                    let vision =
                        vision.min(vision_changes.remove(&p).unwrap_or(FogIntensity::Dark));
            ...
            if !involuntarily
                && transformed_unit.transformed_by_movement(
                    self.get_board(),
                    current,
                    next,
                    distortion,
            ...
            {
                steps.push(EffectStep::Replace(
                    current,
                    *step,
                    Some(EffectData::Unit(transformed_unit.clone())),
                ));
            } else {
        ...
        (
            Effect::new_unit_path(unit.clone(), steps),
            transformed_unit,
            vision_changes,
        )
    }

    pub fn on_unit_normal_action(
        &mut self,
        id: usize,
        path: Path<D>,
        interrupted: bool,
        ...
        let unit = {
            let mut u = self
                .get_game()
                .get_unit(p)
                .expect(&format!("Missing unit at {p:?}"));
            if let Some(i) = unload_index {
                u = u
                    .get_transported()
                    .get(i)
                    .expect(&format!("Missing unit at {p:?}, index {i}"));
            }
            u.clone()
        };
        ...
        first_argument.insert(
            ...
            Dynamic::from(UnitId(id, self.get_observed_unit(id).unwrap().2)),
        );
    ...
    pub fn set_unit_tag(&mut self, position: Point, key: usize, value: TagValue<D>) {
        ...
        let unit = self
            ...
            .expect(&format!("Missing unit at {:?}", position))
            .clone();
        match unit.get_tag(key) {
            Some(old) => {
                self.add_event(Event::UnitReplaceTag(
                    position,
                    TagKeyValues(TagKey(key), [old, value]),
                ));
            }
            _ => {
                self.add_event(Event::UnitSetTag(
                    position,
                    TagKeyValues(TagKey(key), [value]),
                ));
            }
    ...
    pub fn remove_unit_tag(&mut self, position: Point, key: usize) {
        ...
        if let Some(old) = unit.get_tag(key) {
            self.add_event(Event::UnitRemoveTag(
                position,
                TagKeyValues(TagKey(key), [old]),
            ));
            self.update_cached_visibility(position);
    ...
    pub fn set_unit_flag_boarded(&mut self, position: Point, unload_index: usize, flag: usize) {
        ...
        if !unit.get_transported()[unload_index].has_flag(flag) {
            self.add_event(Event::UnitFlagBoarded(
                position,
                unload_index.into(),
                FlagKey(flag),
            ));
    ...
    pub fn set_unit_tag_boarded(
        ...
        let unit = self
            ...
            .expect(&format!("Missing unit at {:?}", position))
            .clone();
        match unit.get_transported()[unload_index].get_tag(key) {
            Some(old) => {
                self.add_event(Event::UnitReplaceTagBoarded(
                    position,
                    unload_index.into(),
                    TagKeyValues(TagKey(key), [old, value]),
                ));
            ...
            _ => {
                self.add_event(Event::UnitSetTagBoarded(
                    position,
                    unload_index.into(),
                    TagKeyValues(TagKey(key), [value]),
                ));
    ...
    pub fn remove_unit_tag_boarded(&mut self, position: Point, unload_index: usize, key: usize) {
        ...
        if let Some(old) = unit.get_transported()[unload_index].get_tag(key) {
            self.add_event(Event::UnitRemoveTagBoarded(
                position,
                unload_index.into(),
                TagKeyValues(TagKey(key), [old]),
            ));
    ...
    pub fn trigger_all_global_events(
        ...
        for (i, conf) in environment.config.global_events.iter().enumerate() {
            ...
            {
                // commander scripts
                match conf.typ.test_global(&*self.get_game()) {
                    Some(first_argument) => scripts.push((script, first_argument)),
                    _ => {
    ...
    pub fn executor<'b>(&'b mut self, mut first_argument: rhai::Map) -> Executor<'b> {
        first_argument.insert(
            CONST_NAME_EVENT_HANDLER.into(),
            Dynamic::from(EventHandlerPointer::from(self)),
        );
