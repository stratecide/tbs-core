...
macro_rules! combat_module {
    ($pack: ident, $name: ident, $d: ty) => {
        ...
        mod $name {
            ...
            pub fn new_defend_script(
                column_name: ImmutableString,
                defender_id: UnitId<$d>,
            ) -> OnDefendScript {
                OnDefendScript {
                    column_name,
                    defender_id,
            ...
            pub fn remember_unit_transported(
            ...
            #[rhai_fn(return_raw)]
            pub fn attacker_bonus(
                ctx: &mut AttackContext,
                defender_id: UnitId<$d>,
                column_id: ImmutableString,
                base_value: Rational32,
            ) -> Result<Rational32, Box<EvalAltResult>> {
                if let Some(ctx) = &mut *ctx.borrow_mut() {
                    Ok(ctx
                        .as_mut()
                        .attacker_bonus(defender_id, column_id, base_value))
                } else {
                    Err("AttackContext isn't supported here".into())
                }
            }
            ...
            pub fn defender_bonus(
                ...
                if let Some(ctx) = &mut *ctx.borrow_mut() {
                    Ok(ctx
                        .as_mut()
                        .defender_bonus(defender_id, column_id, base_value))
                } else {
                    Err("AttackContext isn't supported here".into())
                }
            }
...
impl<'a, 'c, D: Direction> AttackContext<'a, 'c, D> {
    ...
    pub(super) fn executor<'b>(&'b mut self, mut first_argument: rhai::Map) -> Executor<'b> {
        *self.pointer.borrow_mut() = Some(AttackContextPointer::from(self));
        first_argument.insert(
            CONST_NAME_ATTACK_CONTEXT.into(),
            Dynamic::from(self.pointer.clone()),
        );
    ...
    fn remember_unit(&mut self, p: Point, unload_index: Option<i32>) -> Dynamic {
        let unload_index = match (self.handler.get_game().get_unit(p), unload_index) {
            (Some(_), None) => None,
            (Some(unit), Some(i)) if i >= 0 && unit.get_transported().len() > i as usize => {
                Some(i as usize)
            }
            _ => return ().into(),
        };
    ...
    fn on_defend(&mut self, defend_script: OnDefendScript<D>) {
        let Some((pos, unload_index)) = self
            .handler
            .get_observed_unit_pos(defend_script.defender_id.0)
        else {
            return;
        };
        let defender = get_unit(self.handler.get_board(), pos, unload_index).unwrap();
