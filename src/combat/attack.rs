...
use crate::map::map::{NeighborMode, get_line};
use crate::map::point::*;
use crate::map::wrapping_map::OrientedPoint;
use crate::units::hero::{HeroMap, HeroMapWithId};
use crate::units::movement::{Path, PathStep, TBallast};
use crate::units::{UnitData, UnitId, unit::*};
use crate::{dyn_opt, script::*};

use super::{
    AttackCounterState, AttackTargetingFocus, AttackerInfo, AttackerPosition, SplashPattern,
};
...
impl AttackType {
    pub(crate) fn parse_new<'a>(
        s: &'a str,
        loader: &mut crate::config::file_loader::FileLoader,
    ) -> Result<Self, crate::config::ConfigParseError> {
        if s.len() == 0 {
            return Err(crate::config::ConfigParseError::NameTooShort.into());
        }
        if s == "None" {
            return Err(crate::config::ConfigParseError::InvalidColumnValue(
        ...
        Ok(
            match loader.attack_types.iter().position(|name| name == s) {
...
impl DisplaceDirectionModifier {
    fn modify<D: Direction>(&self, dp: &OrientedPoint<D>) -> OrientedPoint<D> {
        ...
        match self {
            ...
            Self::SharpLeft if D::is_hex() => {
                result.direction = result.direction.rotate_times(dp.mirrored, 2)
            }
            Self::BluntLeft if D::is_hex() => {
                result.direction = result.direction.rotate_times(dp.mirrored, 1)
            }
            Self::SharpRight if D::is_hex() => {
                result.direction = result.direction.rotate_times(!dp.mirrored, 2)
            }
            Self::BluntRight if D::is_hex() => {
                result.direction = result.direction.rotate_times(!dp.mirrored, 1)
            }
            Self::SharpLeft | Self::BluntLeft => {
                result.direction = result.direction.rotate(dp.mirrored)
            }
            Self::SharpRight | Self::BluntRight => {
                result.direction = result.direction.rotate(!dp.mirrored)
            }
        }
        result
    ...
    fn from_conf<'a>(
        ...
        Ok((
            match s.trim() {
                ...
                _ => {
                    return Err(crate::config::ConfigParseError::UnknownEnumMember(format!(
                        "DisplaceDirectionModifier::{}",
                        s.to_string()
                    )));
                }
            },
...
impl AttackInstance {
    pub(crate) fn into_executable<D: Direction>(
        ...
        let Some((attacker_pos, _attacker_unload_index)) = attacker_pos.get_position(handler)
        else {
            return Vec::new();
        };
        ...
        match &self.script {
            AttackInstanceScript::Displace {
                ...
                result = targets
                    ...
                    .filter_map(|dp| {
                        ...
                        let distance: i32 = environment
                            .config
                            .unit_attack_bonus(
                                &"PushDistance".to_string(),
                                *distance,
                                handler.get_board(),
                                attack,
                                splash,
                            ...
                            .to_integer();
                        let push_limit: i32 = environment
                            .config
                            .unit_attack_bonus(
                                &"PushLimit".to_string(),
                                *push_limit,
                        ...
                        Some(AttackContextScript {
                            ...
                            priority: None,
                            script: AttackExecutableScript::Displace {
                                throw: *throw,
                                neighbor_mode: *neighbor_mode,
                            },
                            arguments: vec![Dynamic::from(PushArguments {
                                direction: (-defender_id.1)
                                    .update_direction(self.direction_modifier.modify(dp).direction),
            ...
            AttackInstanceScript::Rhai { build_script } => {
                ...
                first_argument.insert(
                    ...
                    Dynamic::from(splash.splash_distance as i32),
                );
                first_argument.insert(
                    CONST_NAME_ATTACKER_ID.into(),
                    attacker_id.map(|id| Dynamic::from(id)).unwrap_or(().into()),
                );
                first_argument.insert(CONST_NAME_ATTACKER.into(), Dynamic::from(attacker.clone()));
                first_argument.insert(
                    CONST_NAME_ATTACKER_POSITION.into(),
                    Dynamic::from(attacker_pos),
                );
                first_argument.insert(
                    CONST_NAME_ATTACK_DIRECTION.into(),
                    Dynamic::from(attack_direction),
                );
                first_argument.insert(
                    CONST_NAME_HEROES.into(),
                    Dynamic::from(heroes_with_ids.clone()),
                );
                first_argument.insert(
                    CONST_NAME_TARGETS.into(),
                    Dynamic::from(
                        targets
                            .into_iter()
                ...
                match executor.run::<D, ()>(*build_script, ()) {
                    ...
                    Err(e) => {
                        environment.log_rhai_error(
                            "AttackSplash preparation",
                            environment.get_rhai_function_name(*build_script),
                            &e,
                        );
...
impl<D: Direction> AttackExecutable<D> {
    ...
    fn execute(
        mut self,
        handler: &mut EventHandler<D>,
        current_team: ClientPerspective,
        heroes: &HeroMap<D>,
        attack_priority: i32,
    ) -> Vec<ScriptedAttack<D>> {
        match self.script {
            AttackExecutableScript::Displace {
                throw,
                neighbor_mode,
            } => {
                let PushArguments {
                ...
                let push_limit = if throw { 0 } else { push_limit };
                let line = get_line(
                    &*handler.get_game(),
                    point,
                    direction,
                ...
                if throw {
                    ...
                    if handler.get_game().get_unit(p).is_none() {
                        ...
                        for (_, distortion) in line.iter().take(line.len() - 1) {
                            path.steps
                                .push(PathStep::Dir(distortion.update_direction(direction)));
                        }
                        handler.unit_path(None, &path, false, true);
                    } else if attacker_team == current_team
                        && get_visible_unit(handler.get_board(), attacker_team, p).is_none()
                    {
                    ...
                    for i in 0..distance {
                        ...
                        if push_count == 0 || blocked.is_some() {
                            ...
                            for (i, p) in tested_points.into_iter().enumerate() {
                                if attacker_team == current_team
                                    && (blocked == Some(i)
                                        || get_visible_unit(handler.get_board(), attacker_team, p)
                                            .is_none())
                                {
                                    handler.effect_fog_surprise(p);
                                }
                            ...
                            for (p, distortion) in line.iter().skip(i).take(push_count) {
                                ...
                                let path = Path::with_steps(
                                    *p,
                                    vec![PathStep::Dir(distortion.update_direction(direction))],
                                );
                                let (path_end, path_distortion) =
                                    path.end(&*handler.get_game()).unwrap();
                                let (path, unit, _additional_vision) =
                                    handler.animate_unit_path(&unit, &path, true);
                                /*if unit.get_team() == current_team {
                                    add_vision(fog_changes, &additional_vision);
                                }*/
                                paths.push(path);
                                units.push((path_end, unit));
                                if let Some(UnitId(id, disto)) =
                                    handler.get_observed_unit_id(*p, None)
                                {
                                    handler.move_observed_unit(
                                        id,
                                        path_end,
                                        None,
                                        disto + path_distortion,
                                    );
                                }
            ...
            AttackExecutableScript::Rhai { ast, script } => {
                ...
                first_argument.insert(
                    CONST_NAME_ATTACKER_POSITION.into(),
                    Dynamic::from(self.attacker_pos),
                );
                ...
                first_argument.insert(
                    CONST_NAME_DEFENDER_POSITION.into(),
                    dyn_opt(self.defender_pos),
                );
                ...
                first_argument.insert(
                    CONST_NAME_ATTACK_PRIORITY.into(),
                    Dynamic::from(attack_priority),
                );
                ...
                match executor.run_ast::<D, Dynamic>(&ast, &script, self.arguments) {
                            ...
                            Ok(result) => {
                                for attack in result {
                                    match attack.try_cast_result::<ScriptedAttack<D>>() {
                                        ...
                                        Err(attack) => {
                                            crate::warn!(
                                                "AttackExecutableScript::Rhai result type {} in '{script}'",
                                                attack.type_name()
                                            );
                    ...
                    Err(e) => {
                        handler.environment().log_rhai_error(
...
pub(super) fn execute_attacks_with_equal_priority<D: Direction>(
    ...
    let mut attacks: Vec<AttackExecutable<D>> = attacks
        .into_iter()
        .filter_map(|(attacker, attack)| {
            ...
            let (input, attack_direction, attack_pattern) =
                attacker.retarget(handler, &attack, &heroes)?;
            ...
            for splash_instance in &attack.splash {
                ...
                for exe in splash_instance.into_executable(
                    handler,
                    &attack,
                    splash_instance,
                    &attacker.attacker_position,
                    attack_direction,
                    &ranges[splash_instance.splash_distance],
                    &heroes,
                    &heroes_with_ids,
                    attacker.temporary_ballast,
