        ...
        match input {
            ...
            AttackInput::SplashPattern(input) => {
                ...
                for d in self.directions.get_dirs(attacker, input) {
                    ...
                    self.points.add_splash_in_direction(
                        game,
                        main_target,
                        range,
                        |i, p, distortion| {
                            let dp = OrientedPoint::new(
                                p,
                                distortion.is_mirrored() != main_target.mirrored,
...
impl SplashDamagePointSource {
    fn add_splash_in_direction<D: Direction>(
        &self,
        game: &Board<D>,
        main_target: OrientedPoint<D>,
        range: usize,
        mut add: impl FnMut(usize, Point, Distortion<D>),
    ) {
        match self {
            Self::AttackPattern => {
                panic!(
                    "Shouldn't call SplashDamagePointSource::AttackPattern.add_splash_in_direction"
                );
            }
            Self::Straight => {
                let points = get_line(
            ...
            Self::TriangleDiagonal => {
                let layers =
                    range_in_layers(game, main_target.point, range, &[main_target.direction]);
            ...
            Self::TriangleStraight => {
                let layers = cannon_range_in_layers(
                    game,
                    main_target.point,
                    range,
                    &[main_target.direction],
            ...
            Self::Rhai(function_index) => {
                ...
                first_argument.insert(
                    ...
                    Dynamic::from(main_target.direction),
                );
                first_argument.insert(
                    CONST_NAME_MIRRORED.into(),
                    Dynamic::from(main_target.mirrored),
                );
...
impl SplashDamageDirectionSource {
    pub fn get_dirs<D: Direction>(&self, attacker: &Unit<D>, input: OrientedPoint<D>) -> Vec<D> {
        match self {
            ...
            Self::UnitAttribute(tag) => match attacker.get_tag(tag.0) {
