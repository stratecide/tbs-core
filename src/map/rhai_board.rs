    ...
    ($name: ident, $d: ty, $board: ty) => {
        ...
        mod $name {
            ...
            pub fn get_neighbor(board: $board, p: Point, d: $d) -> Dynamic {
                map::get_neighbor(board.as_ref(), p, d)
                    .map(|(p, _)| Dynamic::from(p))
                    .unwrap_or(Dynamic::UNIT)
            }

            pub fn get_neighbors(board: $board, p: Point) -> Array {
                map::get_neighbors(board.as_ref(), p, NeighborMode::FollowPipes)
                    .into_iter()
            ...
            pub fn get_neighbors_with_direction(board: $board, p: Point) -> Array {
                <$d>::list()
                    ...
                    .filter_map(|d| {
                        map::get_neighbor(board.as_ref(), p, d)
                            .map(|(p, _)| Dynamic::from(OrientedPoint::simple(p, d)))
                    })
                    .collect()
            }
            ...
            pub fn positions_in_range(board: $board, p: Point, range: i32) -> Array {
                ...
                let mut set: FxHashSet<Point> =
                    get_neighbors_layers(board.as_ref(), p, range as usize)
                        .into_iter()
                        .flatten()
                        .collect();
                set.insert(p);
            ...
            pub fn spread_search(
                ...
                Ok(result.into_iter().map(|p| Dynamic::from(p)).collect())
            }
            ...
            pub fn get_terrain(board: $board, p: Point) -> Terrain<$d> {
                board
                    ...
                    .get_terrain(p)
                    .cloned()
                    .expect("script requested terrain at {p:?}, but that point is invalid")
            }
            ...
            pub fn has_flag(board: $board, owner_id: i32, flag: FlagKey) -> bool {
                ...
                board
                    .as_ref()
                    .get_owning_player(owner_id as i8)
                    .map(|p| p.has_flag(flag.0))
                    .unwrap_or(false)
            }
