...
impl<D: Direction> BoardView<D> for Map<D> {
    ...
    fn get_pipes(&self, p: Point) -> &[PipeState<D>] {
        self.pipes.get(&p).map(Vec::as_slice).unwrap_or(&[])
    }
    fn get_terrain(&self, p: Point) -> Option<&Terrain<D>> {
        self.terrain.get(&p)
    }
    fn get_tokens(&self, p: Point) -> &[Token<D>] {
        self.tokens.get(&p).map(Vec::as_slice).unwrap_or(&[])
    }
    ...
    fn get_team(&self, _: i8) -> ClientPerspective {
...
impl<D: Direction> FieldData<D> {
    pub fn game_field(game: &impl BoardView<D>, p: Point) -> Self {
        Self {
            pipes: game.get_pipes(p).to_vec().try_into().unwrap(),
            terrain: game.get_terrain(p).cloned().unwrap(),
            tokens: game.get_tokens(p).to_vec().try_into().unwrap(),
            unit: game.get_unit(p).cloned(),
        }
    }
...
pub fn valid_points<D: Direction>(board: &impl BoardView<D>) -> Vec<Point> {
...
pub fn width_search<D: Direction>(board: &impl BoardView<D>, start: Point, mut f: Box<&mut dyn FnMut(Point) -> bool>) -> HashSet<Point> {
    ...
    while to_check.len() > 0 {
        ...
        for p in to_check {
            if f(p) {
                result.insert(p);
                for p in get_neighbors(board, p, NeighborMode::Direct) {
                    if !result.contains(&p.point) && !rejected.contains(&p.point) {
                        next.insert(p.point);
                    }
                }
...
pub fn get_neighbors_layers<D: Direction>(board: &impl BoardView<D>, center: Point, range: usize) -> Vec<HashSet<Point>> {
    ...
    let mut result = Vec::new();
    let mut layer: HashSet<(Point, D, Option<D>)> = HashSet::default();
    for dp in get_neighbors(board, center, NeighborMode::FollowPipes) {
        layer.insert((dp.point, dp.direction, None));
    }
    for _ in 1..range {
        let previous_layer = layer;
        ...
        for (p, dir, dir_change) in previous_layer {
            result_layer.insert(p);
            if let Some((point, distortion)) = get_neighbor(board, p, dir) {
                let dir_change = match (distortion.is_mirrored(), dir_change) {
                    (_, None) => None,
                    (true, Some(angle)) => Some(angle.mirror_vertically()),
                    (false, Some(angle)) => Some(angle),
...
pub fn get_income_factor<D: Direction>(map: &Board<D>, owner_id: i8) -> Rational32 {
