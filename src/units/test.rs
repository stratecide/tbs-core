use interfaces::{GameEventsMap, Perspective};
...
fn unit_builder_transported() {
    ...
    let unit: Unit<Direction4> = UnitType::TRANSPORT_HELI
        .instance(&map_env)
        .set_owner_id(0)
        .set_transported(vec![UnitType::MARINE.instance(&map_env).set_hp(34).build()])
        .build();
...
fn fog_replacement() {
    ...
    map.set_terrain(
        Point::new(1, 1),
        TerrainType::City.instance(&map_env).build(),
    );
    let unit = UnitType::SNIPER
        .instance(&map_env)
        .set_owner_id(0)
        .set_flag(FLAG_CAPTURING)
        .build();
...
fn transported_visibility() {
    ...
    let transporter = UnitType::TRANSPORT_HELI
        .instance::<Direction4>(&environment)
        .set_owner_id(0)
        .set_transported(vec![
            UnitType::BAZOOKA
    ...
    assert_eq!(
        1,
        transporter
            .fog_replacement(&board, origin, FogIntensity::TrueSight)
            .unwrap()
            .get_transported()
            .len()
    );
    assert_eq!(
        1,
        transporter
            .fog_replacement(&board, origin, FogIntensity::NormalVision)
            .unwrap()
            .get_transported()
            .len()
    );
    assert_eq!(
        0,
        transporter
            .fog_replacement(&board, origin, FogIntensity::Light)
            .unwrap()
            .get_transported()
            .len()
    );
    assert_eq!(
        None,
        transporter.fog_replacement(&board, origin, FogIntensity::Dark)
    );
}
...
fn drone() {
    ...
    for x in 0..5 {
        for y in 0..7 {
            map.set_terrain(
                Point::new(x, y),
                TerrainType::Sea.instance(&map_env).build(),
            );
        }
    }
    map.set_unit(
    ...
    settings.players[0]
        .get_tag_bag_mut()
        .set_tag(&map_env, TAG_FUNDS, 1000.into());
    let (mut server, events) = Game::new_server(
        map.clone(),
        &settings,
        settings.build_default(),
        Urc::new(|| 0.),
    );
    let mut client = Game::new_client(
        map,
        &settings,
        settings.build_default(),
        events.get(&Perspective::Team(0)).unwrap(),
    );
    assert!(
        server
            .get_unit(Point::new(3, 4))
            .unwrap()
            .get_tag(TAG_DRONE_STATION_ID)
            .is_none()
    );
    let events = server
        .handle_command(
    ...
    assert!(
        !server.get_unit(Point::new(3, 4)).unwrap().get_transported()[0].has_flag(FLAG_EXHAUSTED)
    );
    server
        .handle_command(
            Command::UnitCommand(UnitCommand {
...
fn puffer_fish() {
    ...
    map.set_unit(
        Point::new(1, 1),
        Some(UnitType::PUFFER_FISH.instance(&map_env).build()),
    );
    map.set_unit(
        Point::new(2, 0),
        Some(
    ...
    .unwrap();
    assert_eq!(
        game.get_unit(Point::new(2, 1)).unwrap().typ(),
        UnitType::PUFFER_FISH
    );
...
fn s_factory() {
    ...
    let to_build = UnitType::MARINE
        .instance(&game.environment())
        .set_owner_id(0)
        .set_hp(100);
    game.handle_command(
        Command::UnitCommand(UnitCommand {
            ...
            action: UnitAction::custom(
                ...
                vec![
                    ...
                    CustomActionInput::Direction(Direction6::D180),
                ],
            ),
        }),
        Urc::new(|| 0.),
    ...
    .unwrap();
    assert_eq!(
        *game.get_unit(Point::new(0, 1)).unwrap(),
        to_build.set_flag(FLAG_EXHAUSTED).build()
    );
    assert!(
...
fn marine_movement_types() {
    ...
    map.set_unit(
        Point::new(3, 3),
        Some(UnitType::SNIPER.instance(&map_env).set_owner_id(1).build()),
    );
    let mut settings = map.settings().unwrap();
    settings.players[0]
        .get_tag_bag_mut()
    ...
    assert_eq!(
        ...
        Some(
            &UnitType::MARINE
                ...
                .set_owner_id(0)
                .set_hp(100)
                .set_movement_type(MovementType::AMPHIBIOUS)
                .build()
        )
    );
}
...
fn chess_movement_exhausts_all() {
    let map = chess_board();
    let settings = map.settings().unwrap();
    let mut server = Game::new_server(
        map.clone(),
        &settings,
        settings.build_default(),
        Urc::new(|| 0.),
    ...
    for x in 0..8 {
        ...
        assert!(
            !server
                .get_unit(Point::new(x, 0))
                .unwrap()
                .has_flag(FLAG_EXHAUSTED)
        );
        assert!(
            !server
                .get_unit(Point::new(x, 1))
                .unwrap()
                .has_flag(FLAG_EXHAUSTED)
        );
        if x > 0 {
...
fn chess_castling() {
    ...
    assert!(
        server
            .get_unit(Point::new(0, 0))
            .unwrap()
            .has_flag(FLAG_UNMOVED)
    );
    assert!(
        server
            .get_unit(Point::new(4, 0))
            .unwrap()
            .has_flag(FLAG_UNMOVED)
    );
    assert!(
    ...
    assert!(
        !server
            .get_unit(Point::new(1, 0))
            .unwrap()
            .has_flag(FLAG_UNMOVED)
    );
    assert!(
        !server
            .get_unit(Point::new(2, 0))
            .unwrap()
            .has_flag(FLAG_UNMOVED)
    );
    assert!(
...
fn chess_en_passant() {
    ...
    map.set_unit(
        ...
        Some(
            UnitType::PAWN
                .instance(&map_env)
                .set_owner_id(0)
                .set_tag(TAG_PAWN_DIRECTION, TagValue::Direction(Direction4::D270))
                .set_hp(100)
                .build(),
        ),
    );
    map.set_unit(
        ...
        Some(
            UnitType::PAWN
                .instance(&map_env)
                .set_owner_id(1)
                .set_tag(TAG_PAWN_DIRECTION, TagValue::Direction(Direction4::D90))
                .set_hp(100)
                .build(),
        ),
    );
    ...
    assert_eq!(
        server
            .get_unit(Point::new(0, 2))
            .unwrap()
            .get_tag(TAG_EN_PASSANT),
        Some(TagValue::Point(Point::new(0, 1)))
    );
    assert_eq!(
        server.get_unit(Point::new(0, 2)).unwrap().get_en_passant(),
        Some(Point::new(0, 1))
    );
    server
        .handle_command(
...
fn magnet_pulls_through_pipe() {
    ...
    map.set_pipes(
        Point::new(1, 0),
        vec![PipeState::new(Direction4::D0, Direction4::D270).unwrap()],
    );
    map.set_pipes(
        Point::new(2, 0),
        vec![PipeState::new(Direction4::D0, Direction4::D180).unwrap()],
    );
    map.set_unit(
        Point::new(1, 2),
        Some(
...
fn fog_surprise() {
    ...
    let path = Path::with_steps(
        Point::new(0, 0),
        [PathStep::Dir(Direction4::D0); 6].to_vec(),
    );
    let events = game
        .handle_command(
            Command::UnitCommand(UnitCommand {
    ...
    assert!(
        events
            .get(&Perspective::Team(0))
            .unwrap()
            .contains(&Event::Effect(Effect::new_fog_surprise(Point::new(6, 0))))
    );
}
