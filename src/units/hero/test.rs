...
fn buy_hero() {
    ...
    map.set_unit(
        ...
        Some(
            UnitType::SMALL_TANK
                .instance(&map_env)
                .set_owner_id(1)
                .set_hero(Hero::new(HeroType::CRYSTAL_OBELISK))
                .build(),
        ),
    );
    ...
    game_config.players[0]
        .get_tag_bag_mut()
        .set_tag(&map_env, TAG_FUNDS, 999999.into());
    ...
    assert_eq!(
        Hero::aura_range(
            &board,
            &server.get_unit(Point::new(0, 1)).unwrap(),
            Point::new(0, 1),
            None
        ),
        None
    ...
    let board = Board::new(&server);
    assert_eq!(
        server.get_unit(Point::new(1, 1)).unwrap().get_hero(),
        Some(&Hero::new(HeroType::JAX))
    );
    assert!(
        server
    ...
    assert_eq!(
        Hero::aura_range(
            &board,
            &server.get_unit(Point::new(1, 1)).unwrap(),
            Point::new(1, 1),
            None
        ),
        Some(2)
...
fn crystal() {
    ...
    assert_eq!(
        Hero::aura_range(
            &board,
            &server.get_unit(Point::new(4, 4)).unwrap(),
            Point::new(4, 4),
            None
        ),
        Some(2)
    ...
    let board = Board::new(&server);
    assert_eq!(
        server.get_unit(Point::new(0, 1)),
        Some(
            &UnitType::HERO_CRYSTAL
    ...
    assert_eq!(
        Hero::hero_influence_at(&board, Point::new(0, 0), Some(0)).len(),
        1
    );
    assert_eq!(
        Hero::hero_influence_at(&board, Point::new(0, 0), Some(1)).len(),
        0
    );
    assert_eq!(
        Hero::hero_influence_at(&board, Point::new(0, 0), None).len(),
        1
    );

    assert!(
        aura_damage < power_aura_damage,
        "{aura_damage} < {power_aura_damage}"
    );
    ...
    server
        ...
        .unwrap();
    assert_eq!(
        server.get_unit(Point::new(4, 4)).unwrap().get_hp(),
        100 - 2 * crystal_damage
    );
    server
        .handle_command(
            Command::UnitCommand(UnitCommand {
...
fn earl_grey() {
    ...
    map.set_unit(
        Point::new(2, 1),
        Some(UnitType::MARINE.instance(&map_env).set_owner_id(0).build()),
    );
    map.set_unit(
        Point::new(3, 1),
    ...
    assert_eq!(
        server.get_unit(Point::new(2, 1)).unwrap().movement_points(
            &board,
            Point::new(2, 1),
            None,
            &heroes
        ),
        server.get_unit(Point::new(4, 4)).unwrap().movement_points(
            &board,
            Point::new(4, 4),
            None,
            &heroes
...
fn tess() {
    ...
    settings.players[0]
        .get_tag_bag_mut()
        .set_tag(&map_env, TAG_FUNDS, 9999.into());
    settings.players[1]
        .get_tag_bag_mut()
        .set_tag(&map_env, TAG_FUNDS, 42.into());
    let (server_backup, _) = Game::new_server(
        map.clone(),
        &settings,
        settings.build_default(),
        Urc::new(|| 0.),
    ...
    server
        .handle_command(
            Command::UnitCommand(UnitCommand {
                unload_index: None,
                path,
                action: UnitAction::hero_power(
                    1,
                    vec![
                        CustomActionInput::ShopItem(3.into()),
    ...
    assert!(
        !server
            .get_unit(Point::new(2, 1))
            .unwrap()
            .has_flag(FLAG_EXHAUSTED)
    );
    let remaining_funds = server
        .get_owning_player(0)
        .unwrap()
        .get_tag(TAG_FUNDS)
        .unwrap()
...
fn edwin() {
    ...
    let friend = UnitType::SMALL_TANK
        .instance(&map_env)
        .set_owner_id(0)
        .build();
            ...
            Command::UnitCommand(UnitCommand {
                ...
                action: UnitAction::hero_power(
                    ...
                    vec![
                        ...
                        CustomActionInput::Point(Point::new(0, 4)),
                    ],
                ),
            }),
            Urc::new(|| 0.),
...
fn jax() {
    ...
    map.set_terrain(
        jump_mountain,
        TerrainType::Mountain.instance(&map_env).build(),
    );
    ...
    let (mut server_backup, _) = Game::new_server(
        map.clone(),
        &settings,
        settings.build_default(),
        Urc::new(|| 0.),
    ...
    let options = board
        ...
        .options_after_path(&board, &path, None, &[]);
    assert!(options.contains(&UnitAction::hero_power(1, Vec::new())));
    let script = config.hero_powers(HeroType::JAX)[1]
        .script
        .unwrap()
        .0
        .unwrap();
    ...
    let mut server = server_backup.clone();
    assert_ne!(
        server.get_fog_at(ClientPerspective::Team(0), jump_blocked),
    ...
    for p in get_neighbors(&board, jump_possible, crate::map::map::NeighborMode::Direct) {
        assert_eq!(
            server.get_unit(p.point).unwrap().get_hp() < 100,
            p.direction != Direction4::D90
        );
    }
...
fn kane() {
    ...
    let board = Board::from(&server);
    assert!(!can_see_unit_at(
        &board,
        ClientPerspective::Team(1),
        hero_pos,
        server.get_unit(hero_pos).unwrap(),
        true
    ));
    ...
    server
        .handle_command(
            Command::UnitCommand(UnitCommand {
                ...
                action: UnitAction::hero_power(
                    1,
                    vec![CustomActionInput::Direction(Direction4::D0)],
                ),
            }),
            Urc::new(|| 0.),
        )
